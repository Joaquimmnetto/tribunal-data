...
source("src/R/utils.R")
	...
	names.perf = list(ally='Ally',enemy='Enemy', fuck='thisgaycolor', ec='Cont. Enemy', nc='Non-Cont.', offender='Offender')
	cl.perf = create.color.scale('Groups', names.perf)
	
	#names.cont = list(ally='Ally', enemy='Enemy')
	#cl.cont = create.color.scale('Groups', names.cont)
	...
	plot.topic.offender.perf <- function(col.name){
			plt <- add.theme(
			ggplot() + 
			geom_smooth(data=offender.ally.topics.perf, aes(x=performance, y=get(col.name) * 100, color=names.perf$ally)) +
				geom_point(data=offender.ally.topics.perf, aes(x=performance, y=get(col.name) * 100, color=names.perf$ally), alpha=0.5) + 
			geom_smooth(data=offender.ec.topics.perf, aes(x=values, y=get(col.name) * 100, color=names.perf$ec)) +
				geom_point(data=offender.ec.topics.perf, aes(x=values, y=get(col.name) * 100, color=names.perf$ec), alpha=0.5) +
			geom_smooth(data=offender.nc.topics.perf, aes(x=values, y=get(col.name) * 100, color=names.perf$nc)) +
				geom_point(data=offender.nc.topics.perf, aes(x=values, y=get(col.name) * 100, color=names.perf$nc), alpha=0.5)  
			#+ geom_smooth(data=offender.topics.perf, aes(x=performance, y=get(col.name) * 100, color=names.perf$offender)) 
			#+ geom_point(data=offender.topics.perf, aes(x=performance, y=get(col.name) * 100, color=names.perf$offender), alpha=0.5)
			,col.scale = cl.perf)
	...
	plot.topic.offender.cont <- function(col.name){
		plt <- add.theme(
			ggplot() + 
			geom_smooth(data=offender.ally.topics.cont, aes(x=contamination, y=get(col.name) * 100, color=names.cont$ally),span=1) +
			 geom_point(data=offender.ally.topics.cont, aes(x=contamination, y=get(col.name) * 100, color=names.cont$ally),alpha=0.5) + 
			geom_smooth(data=offender.enemy.topics.cont, aes(x=contamination, y=get(col.name) * 100, color=names.cont$enemy),span=1) +
			 geom_point(data=offender.enemy.topics.cont, aes(x=contamination, y=get(col.name) * 100, color=names.cont$enemy),alpha=0.5)
			,col.scale = cl.perf)
		return(plt)
	}
