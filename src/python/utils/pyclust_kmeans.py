...
def _kmeans_init(X, n_clusters, init_clusters, method='balanced'):
...
def _assign_clusters(X, centers, _metric):
	""" Assignment Step:
		   assign each point to the closet cluster center
	"""
	dist2cents = scipy.spatial.distance.cdist(X, centers, metric=_metric)
	membs = np.argmin(dist2cents, axis=1)

	return (membs)


def _cal_dist2center(X, center, _metric):
	""" Calculate the SSE to the cluster center
	"""
	dmemb2cen = scipy.spatial.distance.cdist(X, center.reshape(1, X.shape[1]), metric=_metric)
	return (np.sum(dmemb2cen))


def _update_centers(X, membs, n_clusters, metric):
	...
	for clust_id in range(n_clusters):
		...
		sse[clust_id] = _cal_dist2center(X[memb_ids, :], centers[clust_id, :], metric)
	return (centers, sse)


def _kmeans_run(X, n_clusters, max_iter, init_clusters, tol, metric):
	...
	centers = _kmeans_init(X, n_clusters, init_clusters)

	sse_last = 9999.9
	n_iter = 0
	for it in range(1, max_iter):
		membs = _assign_clusters(X, centers, metric)
		centers, sse_arr = _update_centers(X, membs, n_clusters, metric)
...
def _kmeans(X, n_clusters, max_iter, n_trials, init_clusters, tol, metric):
	...
	for i in range(n_trials):
		centers, labels, sse_tot, sse_arr, n_iter = _kmeans_run(X, n_clusters, max_iter, init_clusters, tol, metric)
		if i == 0:
...
class KMeans(object):
	"""
		...
		-------
		   ...
		   metric: the metric used in order to calculate the distance between the points. All metrics listed in the scipy.distance package are valid here.
	...
	def __init__(self, n_clusters=2, n_trials=10, init_clusters=None, max_iter=100, tol=0.001, metric='cosine'):
	...
	def fit(self, X):
		...
		self.centers_, self.labels_, self.sse_arr_, self.n_iter_ = \
			_kmeans(X, self.n_clusters, self.max_iter, self.n_trials, self.init_clusters, self.tol)
