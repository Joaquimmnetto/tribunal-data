...
ally_vocab_fn = model_dir+"/ally/vocab_freq.pkl" if len(sys.argv) < 4 else sys.argv[3]
enemy_vocab_fn = model_dir+"/enemy/vocab_freq.pkl" if len(sys.argv) < 5 else sys.argv[4]
offender_vocab_fn = model_dir+"/offender/vocab_freq.pkl" if len(sys.argv) < 6 else sys.argv[5]
words_fn = model_dir+"/full/words.pkl"

ally_vocab = load_pkl(ally_vocab_fn)
enemy_vocab = load_pkl(enemy_vocab_fn)
offender_vocab = load_pkl(offender_vocab_fn)
words = load_pkl(words_fn)
...
def save_vocab_diffs(df_ae, df_ao, df_eo):
	with open(out_dir+"/full_df_ae.csv",'w') as output:
		save_diff(output,df_ae)

	with open(out_dir+"/full_df_ao.csv",'w') as output:
		save_diff(output,df_ao)

	with open(out_dir+"/full_df_eo.csv",'w') as output:
