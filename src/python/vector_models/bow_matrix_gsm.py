...
def build_dict(corpus, lower_cut=1200000, upper_cut=0.75, vocab_trim=20000):
  id2word = Dictionary(GensimCorpus(corpus))
...
def main():  
  gsm_dict = build_dict(CountDocIterator(DocIterator(base.corpus),gensim=True),
                        lower_cut=1200000, upper_cut=0.75, vocab_trim=20000)

  gsm_dict.save(vecs.bow.vocab) 
  
  corpus = CountDocIterator(DocIterator(base.corpus),gensim=True)  
