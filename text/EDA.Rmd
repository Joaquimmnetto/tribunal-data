...
## __matches.csv__
<!--se tiver como fazer com que cada um dos summaries caibam em uma linha, seria ótimo-->
```{r echo=FALSE}
summary(matchs)
```
...
Tempo de jogo dos aliados do ofensor
<!--kazuki, tem como colocar esses dois lado a lado, eu não tou conseguindo D:-->
```{r echo=FALSE}
summary(allies.win["time.played"])
```


Tempo de jogo dos inimigos do ofensor
```{r echo=FALSE}
summary(enemies.win["time.played"])
```
